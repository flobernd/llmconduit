...
async fn reserved_keys_in_extra_body_do_not_duplicate_named_fields() {
    ...
    request.extra_body = std::collections::BTreeMap::from_iter([(
        "parallel_tool_calls".to_string(),
        json!(false),
    )]);
    let _ = collect_stream(gateway.stream_responses(request).await.expect("stream")).await;
...
async fn anthropic_messages_forwards_output_config_as_response_format() {
    ...
    // output_config.effort and the Default profile omits the field. Effort is forwarded
    // and clamped when thinking is on.
    assert_eq!(requests[0].reasoning_effort.as_deref(), None);
